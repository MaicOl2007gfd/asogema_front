...
onMounted(() => {
  document.body.classList.add('wallet-mounted')
})
</script>
  ...
  <div class="wallet-wrap">
    ...
    <main class="wallet-main">
      <div class="wallet-card">
        ...
        <!-- Historial -->
        <section v-if="recargas.length > 0" class="wallet-section">
          <h2 class="wallet-section-title">Historial de recargas</h2>
          <div class="wallet-history">
            <div v-for="rec in recargas" :key="rec.id" class="wallet-history-item">
              <div class="wallet-history-dot" :class="{ ok: rec.estado === 'CONFIRMADO', pend: rec.estado === 'PENDIENTE' }"></div>
              <div class="wallet-history-info">
                <strong>{{ formatCop(rec.monto) }}</strong>
                <span>{{ new Date(rec.created_at).toLocaleString('es-CO', { dateStyle: 'short', timeStyle: 'short' }) }}</span>
              </div>
              <span class="wallet-history-state" :class="rec.estado === 'CONFIRMADO' ? 'ok' : 'pend'">
